...
source "${KS_TOP}/bin/ksconf"   # <— NEW: KS config/option loader
...
if [[ -d ${EXT_DIR} ]] && [[ -f $(realpath -e ${EXT_DIR}/config/${INCONFIG} 2>/dev/null) ]] ; then
  KS_CONFIG_DIR="${EXT_DIR}/config"
else
  __IC=$(basename "$INCONFIG")
  if realpath -e "${KS_CONFIG_DIR}/${__IC}" > /dev/null 2>&1 ; then
    INCONFIG="$__IC"
  else
    die "Can't resolve config file '${INCONFIG}'. Need -D?"
  fi
fi
CFG=$(realpath -e "${KS_CONFIG_DIR}/${INCONFIG}" 2>/dev/null) || die "Bad config spec: ${KS_CONFIG_DIR} : ${INCONFIG}"
...
for i in KS_DEVICE KS_IMAGE KS_PROFILE ; do
  msg "$i ${!i}"; realpath -e ${!i} > /dev/null 2>&1 || die "$i is invalid"
done
...
# ---------------- Hook runner -------------------
runh() {
  local hookdir=$(dirname "$1"); local hook=$(basename "$1"); shift 1
  msg "$hookdir"["$hook"] "$@"
