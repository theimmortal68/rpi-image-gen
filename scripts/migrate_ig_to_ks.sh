...
changed=0
while read -r f; do
  if ! grep -E -q 'RPI_TEMPLATES|BDEBSTRAP_HOOKS|KS_TEMPLATES_RPI|IG_TEMPLATES|IG_HOOKS|META_HOOKS|IGTOP|IGIMAGE|IGDEVICE|IGPROFILE|IGconf_|IGCONF_|\bigconf_' "$f"; then
    continue
  fi
  if [[ $APPLY -eq 1 ]]; then
